...
const EventModal: React.FC<EventModalProps> = ({ event, initialData, onClose, onSave, forceCreate }) => {
  ...
  const levelsForSeries = useMemo(() => {
    ...
    // For "Série Parcours des possibles" and "Série Audiovisuelle", only show levels 1 and 2
    if (badgeSeriesFilter === 'Série Parcours des possibles' || badgeSeriesFilter === 'Série Audiovisuelle') {
  ...
  const filteredBadges = useMemo(() => {
    return availableBadges.filter((badge) => {
      ...
      // For "Série Parcours des possibles" and "Série Audiovisuelle", hide levels 3 and 4
      if ((badge.series === 'Série Parcours des possibles' || badge.series === 'Série Audiovisuelle') && 
