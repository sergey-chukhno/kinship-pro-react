...
import FormationProofHeader from './FormationProofHeader';
import FormationProofConstellation from './FormationProofConstellation';
...
interface FormationProofDetailProps {
...
const FormationProofDetail: React.FC<FormationProofDetailProps> = ({
  proof,
  showPorteurBar = true,
  showRightsLink = true,
}) => {
  ...
  return (
    ...
    >
      ...
      <nav className="pp-tabs" role="tablist" aria-label="Sections de la Preuve Formation">
        {TABS.map((tab) => (
          <button
            key={tab}
            type="button"
      ...
      <div className="pp-body">
        ...
        >
          ...
          <div className="cadre-desc">
            <TruncatableText text={proof.description} id="pfDescText" />
          </div>
          ...
          <div className="kpis">
            ...
            <div className="kpi">
              <div className="kpi-n">{proof.kpis.sessions}</div>
              <div className="kpi-l">Séances</div>
            </div>
            <div className="kpi">
              <div className="kpi-n">{proof.kpis.proofs}</div>
              <div className="kpi-l">Preuves émises</div>
            </div>
            <div className={`kpi${isGold ? ' kpi-gold' : ''}`}>
              <div className="kpi-n">{proof.kpis.hours}</div>
              <div className="kpi-l">{isGold ? `Charge · ${proof.kpis.ects}` : 'Charge de travail'}</div>
            </div>
            <div className={`kpi${isGold ? ' kpi-gold' : ''}`}>
          ...
          <div className="sec-label">Co-attestants — figés à la clôture</div>
          {proof.coAttestants.map((co) => (
            <div key={co.name} className="coatt">
              <div className="coatt-av" style={{ background: co.avatarBg, color: co.avatarColor }}>
                {co.initials}
              </div>
          ...
          <div className="info-row">
            <div className="info-cell">
              <div className="info-k">Séances</div>
              <div className="info-v">{proof.sessionsClosedLabel}</div>
            </div>
            <div className="info-cell">
              <div className="info-k">Référentiel</div>
              <div className="info-v">{proof.referential}</div>
            </div>
          </div>
          <div className="info-row">
            <div className={`info-cell${isGold ? ' info-cell-gold' : ''}`}>
              <div className="info-k">{isGold ? 'Participation · Annexe I' : 'Participation'}</div>
              <div className="info-v">{proof.participation}</div>
            </div>
            <div className={`info-cell${isGold ? ' info-cell-gold' : ''}`}>
              <div className="info-k">{isGold ? 'Langue · Annexe I' : 'Langue'}</div>
        ...
        <div
          ...
          aria-label="Acteurs"
        >
          <FormationProofConstellation proof={proof} />
          <div className="rgpd-note">{proof.rgpdNote}</div>
        </div>
        ...
        >
          <div className="acquis-block">
            <div className="acquis-title">
              {isGold ? 'Acquis d’apprentissage · Annexe I' : 'Acquis d’apprentissage'}
            </div>
            <div className="acquis-text">
              <TruncatableText text={proof.learningOutcomes} id="pfAcquisText" />
            </div>
          </div>
          <div className="pills-row">
            <div className="pill-gold">
              {isGold ? `${proof.kpis.hours.replace('h', ' h')} · ${proof.kpis.ects}` : `${proof.kpis.hours.replace('h', ' h')}`}
            </div>
            <div className="pill-gold">Évaluation : {proof.evaluationType}</div>
          </div>
          <div className="sec-label">Les séances</div>
        ...
        >
          <div className="badges-sum">
            {proof.proofsReceived} preuves reçues par {holderLabel} · {proof.proofsSummary}
          </div>
          <div className="prereq-block">
            <div className="prereq-label">{isGold ? 'Annexe I · optionnel' : 'Prérequis'}</div>
            <div className="prereq-title">{proof.prerequisitesTitle}</div>
            <div className="prereq-text">{proof.prerequisites}</div>
          </div>
          {proof.series.map((serie, idx) => (
            <div key={serie.name} className="serie">
              ...
              <div className={`serie-body ${openSeries[idx] ? 'open' : ''}`}>
                {serie.items.map((item) => (
                  <div key={item.name} className="badge-row">
                    <BadgeHex color={item.orgColor} />
                    <span className="badge-name">{item.name}</span>
                    <span className="badge-arrow">{item.kind} ↗</span>
                  </div>
                ))}
                {serie.extraCount ? (
                  <div className="badge-extra">
                    {serie.extraCount} {serie.extraLabel ?? 'éléments supplémentaires'} ▾
                  </div>
                ) : null}
              </div>
            </div>
        ...
        >
          ...
          {isGold ? (
            <>
              ...
              <div className="comp-sub">Rec. 2022/C 243/02 · Annexe I · 16 renseignés · 11 obligatoires</div>
            ...
            <>
              <div className="comp-title">Le cadre de la preuve</div>
              <div className="comp-sub">Générée automatiquement à la clôture — figée, vérifiable sans compte</div>
              <div className="el-list">
                {proof.frameElements.map((el) => (
                  <div key={el.name} className="el-row">
                    <span className="el-bar el-bar-s" style={{ background: '#003189' }} />
                    <span className="el-nm">{el.name}</span>
                    <span className="el-src">{el.source}</span>
                    <span className="el-ok">✓</span>
                  </div>
          ...
          <div className="integrity">
            ...
            <div className="integrity-d">
              Émettrice et co-attestant figés à la clôture · empreintes des preuves agrégées ·
              manifeste figé · l’empreinte reste inchangée en cas d’effacement des données civiles
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="share-zone">
        ...
        <div className="share-grid">
          ...
          <button type="button" className={`s-btn${isGold ? '' : ' disabled'}`} disabled={!isGold}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={isGold ? '#003189' : '#9ca3af'} strokeWidth="2" aria-hidden="true">
              <circle cx="12" cy="12" r="10" />
              <path d="M2 12h20" />
              <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
            </svg>
            <div className={`s-label${isGold ? '' : ' grey'}`}>Europass</div>
          </button>
...
export default FormationProofDetail;
