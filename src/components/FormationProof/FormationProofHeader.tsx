...
interface FormationProofHeaderProps {
...
export const FormationProofHeader: React.FC<FormationProofHeaderProps> = ({
  proof,
  anonymous = false,
  showHash = true,
    ...
    <div className="pp-header">
      {showPorteurBar && (
        <div className="porteur-bar">
          <span className="porteur-label">Vous consultez votre Preuve Formation®</span>
      ...
      <div className="pp-official">
        ...
        <div className="pp-official-right">
          ...
          <div className="pp-org-trust" style={{ color: proof.orgTrustColor }}>
            {proof.orgTrust}
          </div>
        </div>
      </div>
      ...
      <div className="pp-title-band">
        <h1 className="pp-titre">{proof.formationTitle}</h1>
        <div className="pp-meta">
          {proof.dateRange} · {proof.kpis.hours.replace('h', ' heures')} · {proof.modality}
        </div>
      </div>
      ...
      <div className="pp-identity">
        ...
        <div>
          <div className={`pp-id-name ${levelStyle.idNameAccent ? 'pp-id-name-gold' : ''}`}>
            {displayName}
          </div>
      ...
      <div className="pp-status">
        ...
        <span className="pp-status-text">Preuve Formation® active</span>
        <span className="pp-status-text" style={{ marginLeft: 'auto' }}>
      ...
      {showHash && (
        <div className="pp-hash">
          ...
          <span className="pp-hash-str">attestation · {proof.hashShort} · vérifiable</span>
          <Link to="/verify" className="pp-hash-pill" aria-label="Vérifier l'authenticité de la preuve">
            Vérifier ↗
          </Link>
        </div>
      )}
...
export default FormationProofHeader;
